  ...
  methods: {
    ...
    async getLineDetails() {
      try {
        ...
        this.memoryName =
          this.cloudData.memory !== 0
            ? this.cloudData.memory + this.memory_unit
            : this.calaMemoryList[0]?.value + this.memory_unit;
        // 默认选择防御
    ...
    changeCpu(e) {
      ...
      setTimeout(() => {
        ...
        this.memoryName = this.params.memory + this.memory_unit;
        this.getCycleList();
      }, 0);
    },
    // 切换防御
    ...
    // 切换内存
    changeMemory(e) {
      this.params.memory = e.replace(this.memory_unit, "");
      setTimeout(() => {
        this.getCycleList();
      }, 0);
    },
    ...
    getIpPrice() {
      ...
      ipPrice({ id: this.id, ip_num: this.ipValue })
        .then(async (res) => {
          if (this.isShowLevel) {
            await clientLevelAmount({
              id: this.product_id,
      ...
      try {
        getLineConfig({
          ...
          if (res.data.data.ip && res.data.data.ip.length > 0) {
            this.ipValueData =
              res.data.data.ip.filter((item) => {
                return item.value != this.cloudData.ip_num;
              }) || [];
