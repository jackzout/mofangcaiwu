...
new Vue({
  ...
  methods: {
    ...
    async getLineDetails() {
      try {
        ...
        if (this.lineDetail.bw) {
          ...
          this.bwName = this.params.bw === 'NC' ? lang.actual_bw : (this.params.bw + 'M')
          // 循环生成带宽可选数组
          const fArr = []
        ...
        if (this.lineDetail.ip) {
          const temp_ip = this.cloudData.ip_num === 'NC' ? 'NC' : this.cloudData.ip_num
          if (this.cloudData?.ip_num !== 0) { // 初次回填
    ...
    changeIp(e) {
      if (e === lang.actual_ip) {
        ...
        this.params.ip_num = this.calcIPList.filter(item => {
          if (item.value !== 'NC') {
            return (item.value, item.value.split(',').reduce((all, cur) => {
              all += cur.split('_')[0] * 1
              return all
            }, 0)) === temp
          }
    ...
    getCommonData() {
      ...
      document.title = this.commonData.website_name + '-' + lang.common_cloud_text43
    },
    // 获取自动续费状态
    getRenewStatus() {
      const params = {
    ...
    getImage() {
      ...
      image(params).then(res => {
        if (res.data.status === 200) {
          ...
          this.osIcon = "/themes/clientarea/default/v10/img/" + this.osData[0].name + '.svg'
    ...
    osSelectGroupChange(e) {
      this.osData.map(item => {
        if (item.id == e) {
          ...
          this.osIcon = "/themes/clientarea/default/v10/img/" + item.name + '.svg'
    ...
    getConfigData() {
      ...
      getOrderConfig(params).then(res => {
        if (res.data.status === 200) {
          ...
          if (res.data.data.memory && res.data.data.memory.length > 0 && res.data.data.memory[0].type !== 'radio') { // 范围的时候生成默认范围数组
    ...
    // 磁盘挂载
    handelMount(id) {
      this.$confirm(lang.mf_tip30).then(() => {
    ...
    // 磁盘卸载
    handelUnload(id) {
      this.$confirm(lang.mf_tip32).then(() => {
    ...
    // 打开新增Ip弹窗
    showIpDia() {
      getLineConfig({ id: this.id, line_id: this.cloudData.line.id }).then((res) => {
    ...
    getVpcNetwork() {
      ...
      vpcNetwork({ id: this.id }).then((res) => {
    ...
    handDelVpc(id) {
      this.$confirm(lang.mf_tip34).then(() => {
    ...
    // 提交新增IP
    subAddIp() {
      ipOrder({ id: this.id, ip_num: this.ipValue,upgrade_type: "ip_num" }).then((res) => {
        const orderId = res.data.data.id
        this.isShowIp = false
